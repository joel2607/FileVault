...
// UserRoot is the resolver for the userRoot query.
// It retrieves the top-level files and folders for the currently authenticated user.
func (r *queryResolver) UserRoot(ctx context.Context) (*models.Root, error) {
	user, err := middleware.GetCurrentUser(ctx)
	if err != nil {
		return nil, err
	}
	return r.ShareService.GetUserRoot(ctx, user)
}

// AdminRoot is the resolver for the adminRoot query.
// It retrieves all root-level files and folders for an admin user.
// An optional userID can be provided to filter for a specific user's root.
func (r *queryResolver) AdminRoot(ctx context.Context, userID *string) (*models.Root, error) {
	user, err := middleware.GetCurrentUser(ctx)
	if err != nil {
		return nil, err
	}
